import { saveToolSections } from '@/app/admin/(protected)/tools/actions'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { requireAdmin } from '@/lib/admin'
import { getToolSections, listTools } from '@/lib/tools'
...
export default async function AdminToolSectionsPage({
	searchParams,
}: {
	...
	const [{ error, saved }, sections, tools] = await Promise.all([
		searchParams,
		getToolSections(),
		listTools(),
	])
	...
	return (
		<section className="flex flex-col gap-6">
			<h1 className="text-2xl font-medium">Tool sections</h1>
			{error && <p className="text-sm text-destructive">{error}</p>}
			{saved && <p className="text-sm text-muted-foreground">Saved.</p>}
			<p className="text-sm text-muted-foreground">
				One tool slug per line, in display order. Available slugs:{' '}
				{tools.map((tool) => tool.slug).join(', ') || 'none'}.
			</p>

			<form action={saveToolSections} className="flex flex-col gap-8">
					...
					>
						...
						<label className="flex flex-col gap-1 text-sm text-muted-foreground">
							...
							<textarea
								...
								rows={Math.max(4, section.tools.length + 1)}
								defaultValue={section.tools.map((tool) => tool.slug).join('\n')}
